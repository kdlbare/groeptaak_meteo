...
geom_vline(xintercept=c(80,172,264,355))+
  labs(title = "Meteorological Variables Across Days of the Year",
       x = "Day of the Year",
       y = "Average Value") +
  scale_color_manual(values=c('blue', 'red', 'orange', 'black', 'brown', 'purple','green'))
...
```{r}
#PLOT 7
P7_data <- select(data, c("T", "T_NIGHT", "T_DAY", "VPD", "RAIN", "WIND", "TSOIL"))
chart.Correlation(P7_data)

#PLOT 8
P8_data <- data%>%
  filter(YEAR>2006)%>%
  select(c("T", "T_NIGHT", "T_DAY", "VPD", "RAIN", "WIND", "TSOIL", "NEE", "GPP", "RECO"))
