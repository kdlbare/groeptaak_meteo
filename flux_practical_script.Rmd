...
geom_vline(xintercept=c(80,172,264,355))+
  labs(title = "Meteorological Variables Across Days of the Year",
       x = "Day of the Year",
       y = "Average Value") +
  scale_color_manual(values=c('darkgreen','blue','green', 'red', 'orange', 'black', 'brown', 'purple','#FF00CC'), 
                     labels = c('LW down (daW/m²)', 'rain (mm/dag)', 'SW down (daW/m²)', 'T (°C)',
...
regen <- ggplot(average_rain_per_month, aes(x=MONTH, y=rain))+
  ...
  scale_x_discrete(limits=1:12,
                   name = 'month')+
  scale_y_discrete(name='average rain (mm/month)',
                   limits=c(0,50,100, 150))
regen
```
...
ggplot(P6_data, aes(x=DayOfYear, y=values, col=variables))+
  geom_line()+
  labs(x = 'Day of Year', y= 'average carbon fluxes (kgC/m²/year)')
...
```{r}
#PLOT 7
P7_data <- select(data, c("T", "T_NIGHT", "T_DAY","SWDOWN","LWDOWN", "VPD", "RAIN", "WIND", "TSOIL"))
chart.Correlation(P7_data)

#PLOT 8
P8_data <- data%>%
  filter(YEAR>2006)%>%
  select(c("T", "T_NIGHT", "T_DAY","SWDOWN","LWDOWN", "VPD", "RAIN", "WIND", "TSOIL", "NEE", "GPP", "RECO"))
